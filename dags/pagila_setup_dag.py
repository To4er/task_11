...
from include.config import PAGILA_SCHEMA_URL, PAGILA_SCHEMA_FILE, PAGILA_DATA_URL, PAGILA_DATA_FILE, EXTRACT_DIR, PAGILA_CONN_ID
...
def pagila_setup_dag():
    download_schema_task = BashOperator(
        ...
        bash_command=f"mkdir -p {EXTRACT_DIR} && curl -o {EXTRACT_DIR}/{PAGILA_SCHEMA_FILE} {PAGILA_SCHEMA_URL}"
    )

    download_data_task = BashOperator(
        task_id='download_data_file',
        bash_command=f"curl -o {EXTRACT_DIR}/{PAGILA_DATA_FILE} {PAGILA_DATA_URL}"
    )
